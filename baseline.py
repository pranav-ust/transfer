...
output_data = convert_file(os.path.join("Data", "l1.txt"))

test_input = convert_file(os.path.join("Data","test_app.txt"))
test_output = convert_file(os.path.join("Data","test_l1.txt"))
...
def train_model(model, criterion, optimizer, num_epochs, filename):
	f = open(os.path.join("Logs", "Iterations-" + filename + ".txt"), "w+")
	since = time.time()
	...
	for epoch in range(num_epochs):
		...
		for phase in ['train', 'val']:
			...
			for data in dataloders[phase]:
				...
				array_string = '\n'.join(' '.join('%0.8f' %x for x in y) for y in outputs.data.numpy())
...
for beta in betas:
	values = [0.0, 0.0, 0.0]
	string = str(beta)
	net = Net(input_size, hidden_size1, hidden_size2, num_classes, values)
	...
	torch.save(model_ft.state_dict(), 'app2l1.pkl')
	learning_curve(os.path.join("Logs", "Iterations-" + string + ".txt"), string)
